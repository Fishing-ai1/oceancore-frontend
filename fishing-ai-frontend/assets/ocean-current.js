(function(){
  ...
  function renderPrimary(post){
    ...
    return `<article class="oc-current-chapter" data-social-post-id="${safe(post?.id)}">
      ...
      <div class="oc-current-byline">
        ...
        <div><strong>${safe(author)}</strong><span>${safe(postDate(post))} · ${safe(area)} · Spot Safe</span></div>
  ...
  function renderCamp(post){
    ...
    return `<article class="oc-current-chapter">
      ...
      <div class="oc-current-byline"><div class="oc-current-avatar">${safe(initials(author))}</div><div><strong>${safe(author)}</strong><span>${safe(postDate(post,'5h ago'))} · ${safe(postArea(post,'Bribie Island, QLD'))} · Camping · Boating</span></div></div>
  ...
  function renderMore(posts){
    ...
    if(!remaining.length) return `<div class="oc-current-caught-up"><div>${icon('check-circle-2')}<span><strong>You are caught up</strong><small>Fresh adventures from your crew will appear here.</small></span></div><button type="button" data-section="explore">Explore OceanCore</button></div>`;
  ...
  function renderOceanCurrentHome(){
    ...
    section.innerHTML = `<div class="oc-current-home">
      <div class="oc-current-toolbar"><div class="oc-current-context">${icon('map-pin')}<span><strong>${safe(postArea(primary))}</strong> · ${safe(weather.temp)} · ${safe(weather.wind)}</span></div><button class="oc-current-primary" type="button" data-section="create" aria-label="Log an adventure">${icon('plus')}<span>Log an adventure</span></button></div>
      <div class="oc-current-layout">
        <div class="oc-current-river">${renderPrimary(primary)}${renderCamp(camp)}
          <section class="oc-current-more"><div class="oc-current-section-head"><h3>More from your current</h3><div class="oc-current-tabs" aria-label="Feed filters"><button class="${state.socialFeedMode==='home'?'active':''}" type="button" data-home-feed="home">For you</button><button class="${state.socialFeedMode==='following'?'active':''}" type="button" data-home-feed="following">Following</button><button class="${state.socialFeedMode==='local'?'active':''}" type="button" data-home-feed="local">Local</button><button class="${state.socialFeedMode==='videos'?'active':''}" type="button" data-home-feed="videos">Videos</button></div></div><div id="socialHomeFeed">${renderMore(posts)}</div></section>
        </div>
        <aside class="oc-current-signals" aria-label="Live community activity"><div class="oc-current-signals-head"><h3>Live from the Coast</h3><span class="oc-current-live">All activity</span></div><div class="oc-current-timeline">${renderSignals(posts)}</div><button class="oc-current-crew-card" type="button" data-section="community">${icon('users')}<span>Create a crew<small>Fishing, boating and camping together</small></span>${icon('chevron-right')}</button></aside>
      </div>
    </div>`;
  ...
  function decorateChrome(){
    ...
    const navIcons = {home:'home',explore:'compass',watch:'play-square',create:'plus-square',community:'users',messages:'message-square',ai:'brain',profile:'user'};
    ...
    document.querySelectorAll('.nav-group').forEach((group,index)=>{
      ...
      summary.innerHTML = `${icon(index === 0 ? 'fish-symbol' : 'menu')}<span>${index === 0 ? 'Tools' : 'More'}</span>`;
    });
